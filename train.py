...
def main():
    ...
    pyramidnet_model, ema_pyramidnet_model = build_model()
    print(f"Build `{config.model_arch_name}` model successfully.")

    pixel_criterion = define_loss()
    print("Define all loss functions successfully.")

    optimizer = define_optimizer(pyramidnet_model)
    ...
    for epoch in range(start_epoch, config.epochs):
        train(pyramidnet_model,
              ...
              scaler,
              writer)
        acc1 = validate(ema_pyramidnet_model, valid_prefetcher, epoch, writer, "Valid")
        print("\n")
        ...
        save_checkpoint({"epoch": epoch + 1,
                         "best_acc1": best_acc1,
                         "state_dict": pyramidnet_model.state_dict(),
                         "ema_state_dict": ema_pyramidnet_model.state_dict(),
...
def build_model() -> [nn.Module, nn.Module]:
    pyramidnet_model = model.__dict__[config.model_arch_name](alpha=config.model_alpha,
                                                              num_classes=config.model_num_classes)
    pyramidnet_model = pyramidnet_model.to(device=config.device)
    ...
    ema_pyramidnet_model = AveragedModel(pyramidnet_model, avg_fn=ema_avg)

    return pyramidnet_model, ema_pyramidnet_model
...
def train(
    ...
    while batch_data is not None:
        ...
        with amp.autocast():
            output = model(images)
            loss = torch.mul(config.loss_weights, criterion(output, target))
